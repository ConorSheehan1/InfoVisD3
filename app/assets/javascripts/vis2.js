window.onload = function () {
  ...
  // from notes
  const margin = {
    top: 10, right: 20, bottom: 25, left: 250,
  };

  // Width and height
  const outer_width = 1500;
  const outer_height = 300;
  const svg_width = outer_width - margin.left - margin.right;
  const svg_height = outer_height - margin.top - margin.bottom;
  ...
  getVis = function (year_data) {
    ...
    for (i in year_data) {
      let gov = year_data[i].Government;
      const countryx = codes[year_data[i].Country];
      const colorx = colors[gov_types.indexOf(gov)];
  ...
  function getBarChart(year_data) {
    ...
    year_data = d3.nest()
      .key((d) => d.Government)
      .rollup((v) => d3.mean(v, (d) => d.GDP))
      .entries(year_data);
    // key is now governement type, values is average GDP


    // Update the domain of the x scale
    yScale.domain(year_data.map((d) => d.key));
    ...
    bars.transition()
      ...
      .style('fill', (d) => colors[gov_types.indexOf(d.key)]);
    ...
    // make gov a valid class replace space with underscore and remove apostrophe
      .attr('gov', (d) => d.key.replace(/\s+/g, '_').replace(/'/g, ''))
      .attr('x', 2)
      .attr('y', (d) => yScale(d.key))
      .attr('width', (d) => xScale(+d.values))
      .attr('height', yScale.rangeBand())
    ...
    bars.on('mouseover', function (d) {
      ...
      let gov = d3.select(this).attr('gov');
      gov = gov.replace(/\s+/g, '_').replace(/'/g, '');
  ...
  update_data = function () {
    ...
    d3.csv(datasrc, (error, data) => {
      if (error) {
        console.log('error loading csv');
      } else {
        dataset = sort_data(data);
        gov_types = d3.map(dataset[0].values, (d) => d.Government).keys();
        updateCounter++;
      }
  ...
  d3.csv('governments_grouped.csv', (error, data) => {
    if (error) {
      console.log('error loading csv');
    } else {
      ...
      // add text for year
      d3.select('#map-container svg').append('text').attr('id', 'year').attr('x', 5)
        .attr('y', 350)
